...
public class Einstein {
    ...
    public boolean isGameEnd(){//ゲームの終了条件を満たしているかチェック
        ...
        if((Board[4][4]!=0 && Board[4][4]<=6) || player2.size()==0)player1_win=true;//先手の勝利条件確認
        if(Board[0][0]>6 || player1.size()==0)player2_win=true;//後手の勝利条件確認
        if(player1_win)System.out.println("先手が勝ちました");
        if(player2_win)System.out.println("後手が勝ちました");
        System.out.print("\u001b[m");
    ...
    public void move(){//入力から移動先を決定して駒を動かす
        ...
        while(((!move1&&dir==1)||(!move2&&dir==2)||(!move3&&dir==3))||(dir!=1&&dir!=2&&dir!=3))dir=scan.nextInt();//指定した方向が入力されなかった場合もう一度入力してもらう
        if(turn==1){
            for(int i=0;i<5;i++){
                for(int j=0;j<5;j++){
    ...
    public void changeBoard(int row, int col){//move()に呼び出されて実際に移動を行う
        if(turn==1){
            if(Board[row][col]==0)Board[row][col]=move_piece;
            ...
            if(Board[row][col]==0)Board[row][col]=move_piece+6;
            else if(Board[row][col]<=6){
    ...
    public void checkMoving(){//移動できるかチェックする
        ...
        if(turn==1){
            ...
            if(now_row+1<5)move1=true;//下に進めるか
            else move1=false;
            if(now_col+1<5)move3=true;//右に進めるか
            else move3=false;
            if(move1 && move3)move2=true;//右下に進めるか
            else move2=false;
        }
        else{
            for(int i=0;i<5;i++){
            ...
            if(now_col>0)move1=true;//左に進めるか
            else move1=false;
            if(now_row>0)move3=true;//上に進めるか
            else move3=false;
            if(move1 && move3)move2=true;//左上に進めるか
            else move2=false;
        }
    }
    ...
    public void decidePiece(){//動かす駒を決定
        ...
        if(turn==1){
            ...
            for(int i=0;i<player1.size();i++){
                int dis=Math.abs(player1.get(i)-diceroll);
                if(dis==min_dis)piece_candidate.add(player1.get(i));
            }
        }
        else{
            ...
            for(int i=0;i<player2.size();i++){
                int dis=Math.abs(player2.get(i)-diceroll);
                if(dis==min_dis)piece_candidate.add(player2.get(i));
            }
        }
    ...
    public void pieceAnnounce(ArrayList<Integer> piece_candidate){//動かすコマのうちどちらを動かすか選択するようにアナウンス
        ...
        if(piece_candidate.size()==1){
            ...
            for(int i=0;i<piece_candidate.size();i++){
                if(i==0)System.out.print(piece_candidate.get(i));
                else System.out.print("，" + piece_candidate.get(i));
            }
    ...
    public void choicePiece(ArrayList<Integer> piece_candidate){//動かす駒を入力してもらう
        ...
        if(piece_candidate.contains(piece_num))move_piece=piece_num;
        else pieceAnnounce(piece_candidate);
    }
